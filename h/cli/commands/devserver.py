...
def devserver(https, web, ws, worker, assets, beat):
    ...
    if web:
        ...
        m.add_process('web', 'gunicorn --name web --reload --paste conf/app.ini %s' % gunicorn_args)

    if ws:
        # m.add_process('ws', 'gunicorn --name websocket --reload --paste conf/development-websocket.ini %s' % gunicorn_args)
        m.add_process('ws', 'gunicorn --name websocket --reload --paste conf/websocket.ini %s' % gunicorn_args)
